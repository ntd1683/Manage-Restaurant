                ...
                <div class="p-6">
                    <div class="flex flex-wrap justify-between items-center gap-4">
                        <div class="relative lg:flex ">
                            ...
                            <ul class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm"
                                id="options-search" role="listbox">
                            </ul>
                        </div>

                        <div class="flex flex-wrap items-center gap-2">

                            <a href="admin-customers-add.html"
                               class="px-6 py-2.5 inline-flex text-white text-sm rounded-md bg-primary ">
                                <i data-lucide="plus" class="w-5 h-5 inline-flex align-middle me-2"></i>
                                {{ __("Add a New Staff") }}
                            </a>

                            <div class="relative">
                                <span class="absolute top-1/2 start-2.5 -translate-y-1/2"><i data-lucide="calendar-days"
                                                                                             class="h-4 w-4 text-default-700"></i></span>
                                <span class="absolute top-1/2 end-2.5 -translate-y-1/2"><i data-lucide="chevron-down"
                                                                                           class="h-4 w-4 text-default-700"></i></span>
                                <input type="text" name="date_join"
                                       class="py-2.5 w-40 px-9 block bg-default-100 rounded-md border-0 text-sm text-default-700 font-medium focus:border-default-200 focus:ring-0"
                                       id="datepicker-basic">
                            </div><!-- end relative -->


                            <div class="hs-dropdown relative inline-flex md:hidden">
                                <button type="button" id="button_select_level"
                                ...
                                <div
                                    ...
                                    <ul class="flex flex-col gap-1">
                                        <li>
                                            <a class="select-level flex items-center gap-3 font-normal py-2 px-3 transition-all text-default-700 bg-default-100 rounded"
                                               data-value="-1">{{ __("All") }}</a></li>
                                        @foreach($levels as $level => $value)
                                            <li>
                                                <a class="select-level flex items-center gap-3 font-normal text-default-600 py-2 px-3 transition-all hover:text-default-700 hover:bg-default-100 rounded"
                                                   data-value="{{ $value }}">{{ $level }}</a></li>
                                        @endforeach
                                    </ul>
                                </div>
                            </div>
                    ...
                    <form action="">
                        <div class="hs-dropdown relative inline-flex top-3 w-full justify-end hidden md:flex">
                            <div class="ml-3 flex items-center gap-6">
                                <x-forms.inputs.radio class="select-level" name="level" title="ALL" data-value="-1"
                                                      value="-1" id="level-all" checked/>
                                @foreach($levels as $level => $value)
                                    <x-forms.inputs.radio class="select-level" name="level" :title="$level"
                                                          :data-value="$value" :value="$value" :id="'level-' . $level"/>
                                @endforeach
                            </div>
                        </div>
                    </form>
